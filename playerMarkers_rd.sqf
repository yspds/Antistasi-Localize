...
while {true} do {
	...
	// show players or player's vehicles
	{
		...
		if(
			...
			if(_unit getVariable ["INCAPACITATED",false] || {lifeState _unit isEqualTo "INCAPACITATED"} || {_unit getVariable ["ACE_isUnconscious", false]}) then {
				_injured = true;
				_show = false;
			};
		};
		...
		_marker setMarkerTypeLocal "mil_triangle";
		_marker setMarkerSizeLocal [0.8,1];
		_marker setMarkerTextLocal format ["%1 Injured",name _x];
