...
if (isMultiplayer) then
	{
	if (paramsArray select 8 == 1) then {[] execVM "playerMarkers_rd.sqf"};
	};
if (!hayACE) then
	{
			...
			if (!isNil "_owner") then
				...
				if (_owner isEqualType "") then
					...
					if ({getPlayerUID _x == _owner} count (units group player) == 0) then
						{
						hint "你不能登上其他玩家的车辆，如果你们不在同一个小队中";
						moveOut _unit;
						_exit = true;
						};
					};
...
caja addAction ["转移车辆库存到军火库", "[] call A3A_fnc_vaciar"];
caja addAction ["移动此物品", "moveHQObject.sqf",nil,0,false,true,"","(_this == theBoss)"];
bandera addAction ["基地管理", {[] execVM "Dialogs\dialogHQ.sqf"},nil,0,false,true,"","(_this == theBoss) and (petros == leader group petros)"];
bandera allowDamage false;
bandera addAction ["招募单位", {if ([player,300] call A3A_fnc_enemyNearCheck) then {hint "附近有敌人时不能使用"} else {nul=[] execVM "Dialogs\unit_recruit.sqf"}},nil,0,false,true,"","(isPlayer _this) and (_this == _this getVariable ['owner',objNull]) and (side (group _this) == buenos)"];
bandera addAction ["购买车辆", {if ([player,300] call A3A_fnc_enemyNearCheck) then {hint "附近有敌人时不能使用"} else {nul = createDialog "vehicle_option"}},nil,0,false,true,"","(isPlayer _this) and (_this == _this getVariable ['owner',objNull]) and (side (group _this) == buenos)"];
if (isMultiplayer) then {bandera addAction ["个人车库", {nul = [true] spawn A3A_fnc_garage},nil,0,false,true,"","(isPlayer _this) and (_this == _this getVariable ['owner',objNull]) and (side (group _this) == buenos)"]};
bandera addAction ["移动此物品", "moveHQObject.sqf",nil,0,false,true,"","(_this == theBoss)"];
cajaVeh allowDamage false;
cajaveh addAction ["治疗 修理 重新武装", "healandrepair.sqf",nil,0,false,true,"","(isPlayer _this) and (_this == _this getVariable ['owner',objNull]) and (side (group _this) == buenos)"];
cajaveh addAction ["移动此物品", "moveHQObject.sqf",nil,0,false,true,"","(_this == theBoss)"];
...
petros addAction ["请求任务", {nul=CreateDialog "mission_menu";},nil,0,false,true,"","_this == theBoss"];
...
[] spawn {_this call compile preProcessFileLineNumbers "fn_keyhandler.sqf"};

// External members list by [GADD] Monkeynutz
call compile preprocessFileLineNumbers "membership\AutoAddMembers.sqf";